...
def custom_get_average_of_percents(starts, start, end, frame="f1"):
	sum_in_frame = 0	
	num = 0
	for j in range(start, end, 3): # i is in frame
		...
		if (all_f !=0):
			if (frame == "f1"):
				sum_in_frame += f1/all_f
			if (frame == "f2"):
				sum_in_frame += f2/all_f
			if (frame == "f3"):
				sum_in_frame += f3/all_f
	if (num == 0):
		return 0.0
	return (sum_in_frame/num) 

def get_drop(loc, starts): 
	before_avg = custom_get_average_of_percents(starts,max(0,loc-3),loc,"f1")
	after_avg = custom_get_average_of_percents(starts,loc,min(len(starts)-3,loc+3),"f1")
	drop = round(before_avg - after_avg, 3)
	return drop
...
outFile = open(sys.argv[1] + "_with_found_frameshifts_approach_3", 'w')
#badFile = open(sys.argv[1] + "_without_threshold_frameshifts_approach_3", 'w')
...
while gene:
	...
	current_gene = gene
	gene = inFile.readline().strip()
	#if (current_gene != "YIL041W"):# TODO test this one, why stop so far away?  wrong frame??? # TODO: example of "bad" low read regions with false positives..."YHR174W"): #TODO: test this one, why nothing found??? "YDR382W"):
	#	continue
	...
	for i in range(0, len(ss) - 3, 3): # i is in frame
		...
		if (drop_value > 0): # there is a decrease
			...
			new_length = int( 3 * round( (plus_1_stop - drop) / 3. ))
			before_percents = np.array(custom_get_range_of_percents(ss, max(0, drop - new_length), drop, "f1"))
			...
			new_length = int( 3 * round( (minus_1_stop - drop) / 3. ))
			before_percents = np.array(custom_get_range_of_percents(ss, max(0, drop - new_length), drop, "f1"))
			t_test = stats.ttest_ind(before_percents, after_percents, equal_var=False)
