...
for name in genePlusOneNames:
	...
	p_value = random.randint(idx,max_idx)/len(all_sums) # sum 
	print(str(speed) + " " + str(idx) + " " + str(max_idx) + " " +  str(p_value))
	ts = genesOutputInfo[name]
	if (p_value <= 0.05):  # THE P-value cutoff!!!!!
		genesOutFile.write(ts[0] + "," + ts[1] + "," + ts[2] + "," + ts[3] + "," + ts[4] + "," + str(round(float((ts[5])))) + "," + str(round(p_value,6)) + "\n")
print(len(sorted_all_sums))

my_set = set(sorted_all_sums)
new_list = list(my_set)
sorted_list = sorted(new_list)
print(sorted_list)
print(sorted_all_sums)
genesOutFile.close()
...
for i in range(1, len(all_sums)):
	...
	pvaluesOutFile.write(str(round(all_sums[i],6)) + ",")
