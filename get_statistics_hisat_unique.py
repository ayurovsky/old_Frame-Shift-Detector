...
for line in samFile:
	if (line.startswith("@")):
		continue
	ts = line.strip().split()
	# remove non-primary first assignment - as reported by hisat
	if (prev_name == ts[0]):
		#print("skipping " + line)
		continue
	prev_name = ts[0]
	# check that mapping quality is not 0 - hisat returns 60 for unique, 1 for multiple alignments
	if (int(ts[4]) != 60):
		continue
	...
	# we expect the reads that belong to genes on the minus strand to be in reverse complement, for plus strand in forward complement	
	frame = -1
	if (int(ts[1]) & 0x10):
		if (Orfs_Seq_Dict[gene]["strand"] == "+"):
			continue
